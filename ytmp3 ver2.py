...
def sanitize_url(url):
    if "music.youtube.com" in url:
        corrected = url.replace("music.youtube.com", "www.youtube.com")
        print(f"Converted to standard YouTube URL:\n{corrected}")
        return corrected
    return url

def sanitize_url(url): #removes music.youtube error and hhtps error 
    if "music.youtube.com" in url:
        url = url.replace("music.youtube.com", "www.youtube.com")
        print(f"Converted to standard YouTube URL:\n{url}")

    # Remove problematic `&si=` or `?si=` query params
    url = re.sub(r"([&?])si=[^&]+", "", url)
    url = url.rstrip("&?")  # Remove trailing & or ? if any
    return url
